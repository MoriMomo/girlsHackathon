...
- [ ] 4. **X post** on dedicated project account ([@LedgrAppBOT](https://x.com/LedgrAppBOT)), tagging **@BOTChain_ai**, link pasted in the form
- [ ] 5. **Active X presence** ([@LedgrAppBOT](https://x.com/LedgrAppBOT)): at least **5 valid posts in the 30 days** before submission (start now!)
...
## Dedicated Project Socials
- **X Account:** [@LedgrAppBOT](https://x.com/LedgrAppBOT)
...
3. **Publish the frontend**
   ...
   - Get a domain (free via the GitHub Student Pack or a $1 to $1.50 one). Point it at Pages (Settings -> Pages -> Custom domain + DNS records). Keep any receipt for reimbursement.
   - When serving from the domain root, change `base` in `vite.config.js` back to `'/'` and rebuild.

4. **X post (item 4)**: post a screenshot or clip of the working page on [@LedgrAppBOT](https://x.com/LedgrAppBOT), tag **@BOTChain_ai**, paste the link in the form. This is Post 5 in the series below.

5. **Mainnet launch announcement (item 6)**: publish the launch write-up on X/Medium/your site; the title or body must state ledgr is officially launched on BOT Chain Mainnet.

6. **Submit** all 7 items in the form before the deadline.

## X post series (5 posts, ~1/day on @LedgrAppBOT, each tagging @BOTChain_ai)
...
```
ledgr is coming together:
1. Connect wallet
2. Log expense: amount, note, date & category
3. Reads straight from the blockchain
4. Interactive category spend chart
...
Snap a receipt -> AI reads amount, merchant & date -> one tap saves it on-chain.
